...
# Filter to just colonized countries 
colonized <- newdata %>%
  filter(colonized == 1)
...
ggplot(newdata, aes(x = colonized, y = val)) + 
  geom_boxplot() +
...
fit_multiple <- lm(val ~ years_indep + IndViol + the_per_gdp_mean + dah_total_ppp_mean + ghes_total_ppp_mean, colonized)

model = c(years_indep = "Years of Independence", 
          IndViol = "Violent Independence", 
          the_per_gdp_mean = "Total Health Spending per GDP", 
          dah_total_ppp_mean = "Development Assistance for Health (thousands of 2019 PPP USD)",
          ghes_total_ppp_mean = "Government Spending (thousands of 2019 PPP USD)")
